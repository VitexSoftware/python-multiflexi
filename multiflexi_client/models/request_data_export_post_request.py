...
class RequestDataExportPostRequest(BaseModel):
    """
    RequestDataExportPostRequest
    """ # noqa: E501
    action: StrictStr
    format: Optional[StrictStr] = 'json'
    __properties: ClassVar[List[str]] = ["action", "format"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['export', 'status']):
            raise ValueError("must be one of enum values ('export', 'status')")
        return value
